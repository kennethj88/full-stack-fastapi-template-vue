...
const googleLogin = async () => {
  try {
    ...
    const client = (window as Window).google.accounts.oauth2.initTokenClient({
      ...
      callback: async (response) => {
        ...
        try {
          console.log('Google OAuth token received:', response.access_token)
          const result = await auth.googleLogin(response.access_token!)
          
          if (result.requires_password) {
            emit('requires-password', {
              email: result.email,
          ...
          toast.showToast({ 
            message: error.response?.data?.detail || 'Failed to login with Google',
            type: 'error' 
          })
        }
      },
